    ...
    return (
        <nav className="p-4 md:px-6 md-:py-6" aria-label="Main">
            <div className="mx-auto flex max-w-6xl flex-col justify-between py-2 font-medium text-white md:flex-row md:items-center">
                ...
                {/* Mobile Nav */}
                <div className={clsx(
                    "gap-4 fixed bottom-0 top-0 right-0 left-0 z-40 flex flex-col items-end bg-[#070815] pr-4 pt-14 transition-transform duration-300 ease-in-out motion-reduce:transition-none md:hidden",
                ...
                <ul className="hidden md:flex gap-6">
                    {settings.data.navigation.map((item) => {
                        ...
                        return (
                            <li key={item.label}>
                                <PrismicNextLink field={item.link} aria-current={
                                    pathname.includes(asLink(item.link) as string)
                                        ? "page"
                                        : undefined
