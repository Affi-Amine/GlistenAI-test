...
import { Content, PrismicError, isFilled } from "@prismicio/client";
import { PrismicNextImage } from "@prismicio/next";
import { PrismicImage, PrismicLink, PrismicRichText, PrismicText, SliceComponentProps } from "@prismicio/react";
...
const CaseStudies = async ({ slice }: CaseStudiesProps): Promise<JSX.Element> => {
  ...
  const caseStudies = await Promise.all(
    slice.primary.case_study_scope.map(async (item) => {
    ...
    >
      ...
      <div className="mt-20 grid gap-16">
        {caseStudies.map(
          (caseStudy, index) =>
            caseStudy && (
              <div key={caseStudy.id} className="relative grid gap-4 opacity-85 transition-opacity duration-300
              hover:cursor-pointer hover:opacity-100 md:grid-cols-2 md:gap-8 lg:grid-cols-3">
                <div className="flex col-span-1 flex-col justify-center gap-4">
                  ...
                  <PrismicLink document={caseStudy} className="after:absolute after:inset-0 hover:underline">
                    Read <PrismicText field={caseStudy.data.company} /> case study
                  </PrismicLink>
                </div>
                <PrismicNextImage field={caseStudy.data.logo_image} className={
                  clsx(
                    "rounded-xl lg:col-span-2 glass-container gap-4 bg-gradient-to-b from-gray-800 to-gray-900 p-4",
                    (index % 2) && "md:-order-1",
                  )
                } />
              </div>
